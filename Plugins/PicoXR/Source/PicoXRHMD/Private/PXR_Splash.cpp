...
void FPicoXRSplash::OnPreLoadMap(const FString&)
{
	if (PicoSettings->bSplashScreenAutoShow)
	{
		UE_LOG(LogHMD, Log, TEXT("FPicoXRSplash::OnPreLoadMap"));
		PXR_LOGD(PxrUnreal, "FPicoXRSplash::OnPreLoadMap");
		if (!bShown)
		{
			Show();
		}	
...
void FPicoXRSplash::OnPostLoadMap(UWorld*)
{
	...
	{
		UE_LOG(LogHMD, Log, TEXT("FPicoXRSplash::OnPostLoadMap Hide Auto Splash"));
		HideLoadingScreen();
	}
}
...
void FPicoXRSplash::StartTicker()
{
	...
	if (!Ticker.IsValid())
	{
		...
		ExecuteOnRenderThread([this]()
        {
            ...
            Ticker->Register();		
			bTickerStarted = true;
			UE_LOG(LogHMD, Log, TEXT("Splash StartTicker!"));
        });	
	}
}
...
void FPicoXRSplash::StopTicker()
{
	ExecuteOnRenderThread([this]()
	{
		if (Ticker.IsValid())
		{
			...
			bTickerStarted = false;
			bShown = false;
			UE_LOG(LogHMD, Log, TEXT("Splash StopTicker!"));
        }
	});	
	ReleaseTextures();
}
...
{
	UE_LOG(LogHMD, Log, TEXT("Splash ShowLoadingScreen!"));
...
{
	UE_LOG(LogHMD, Log, TEXT("Splash HideLoadingScreen!"));
...
void FPicoXRSplash::RenderFrame_RenderThread(FRHICommandListImmediate& RHICmdList)
{
	...
	{
		UE_LOG(LogHMD, Log, TEXT("Splash WaitToBeginFrame %u"), XFrame->FrameNumber);
		PXR_LOGD(PxrUnreal, "Splash WaitToBeginFrame %u", XFrame->FrameNumber);
	...
	ExecuteOnRHIThread([this,XFrame]()
		{
			...
			{
				UE_LOG(LogHMD, Log, TEXT("Splash BeginFrame %u"), XFrame->FrameNumber);
				PXR_LOGD(PxrUnreal, "Splash BeginFrame %u", XFrame->FrameNumber);
	...
	ExecuteOnRHIThread([this, XFrame]()
		{
			if (XFrame->ShowFlags.Rendering)
			{
				UE_LOG(LogHMD, Log, TEXT("Splash EndFrame %u"), XFrame->FrameNumber);
				PXR_LOGD(PxrUnreal, "Splash EndFrame %u", XFrame->FrameNumber);
