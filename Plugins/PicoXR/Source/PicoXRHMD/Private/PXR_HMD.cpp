...
bool FPicoXRHMD::Initialize()
{
    ...
    FString UnrealSDKVersion = "UE4_2.2.0.4";
	...
	UE_LOG(LogHMD, Log, TEXT("PxrUnreal xrVersion: %s"), *UnrealSDKVersion);
...
void FPicoXRHMD::ProcessControllerEvent(const PxrEventDataControllerChanged EventData)
{
	...
	case PXR_DEVICE_MENUPRESSED_STATE:
		{
			PXR_LOGD(PxrUnreal, "ProcessControllerEvent PXR_DEVICE_MENUPRESSED_STATE Stage : %d,StageProgress : %d",EventData.status, EventData.varying[0]);
			EventManager->HandMenuPressedDelegate.Broadcast(EventData.status,EventData.varying[0]);
			break;
		}
	case PXR_DEVICE_HANDTRACKING_SETTING:
		{
			PXR_LOGD(PxrUnreal, "ProcessControllerEvent PXR_DEVICE_HANDTRACKING_SETTING status : %d",EventData.status);
			EventManager->HandTrackingEnabledDelegate.Broadcast(EventData.status);
			break;
		}
	default:
		break;
...
void FPicoXRHMD::OnBeginPlay(FWorldContext& InWorldContext)
{
	if (PicoSplash)
	{
		PicoSplash->StopTicker();//Prevent image resources from being unavailable when Splash is added at the BeginPlay.TODO://it's a bug
	}
...
uint32 FPicoXRHMD::CreateMRCStereoLayer(FTextureRHIRef BackgroundRTTexture, FTextureRHIRef ForegroundRTTexture)
{
	...
	if (CurrentMRCLayer)
	{
		CurrentMRCLayer->SetLayerState(EPicoXRLayerState::Unknown);
	}
