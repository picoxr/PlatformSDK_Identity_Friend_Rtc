...
void FPICOXRStereoLayer::SetPXRLayerDesc(const IStereoLayers::FLayerDesc& InDesc)
{
	...
	if (LayerDesc.TextureObj != InDesc.TextureObj || LayerDesc.LeftTextureObj != InDesc.LeftTextureObj)
	{
...
void FPICOXRStereoLayer::CreateUnderlayMesh(TArray<FVector>& Vertices, TArray<int32>& Triangles, TArray<FVector2D>& UV0)
{
	if (LayerDesc.HasShape<FQuadLayer>())
	{
		...
		FIntPoint TexSize = LayerDesc.TextureObj.IsValid()&&LayerDesc.TextureObj->GetResource() ? FIntPoint(LayerDesc.TextureObj->GetResource()->GetSizeX(),LayerDesc.TextureObj->GetResource()->GetSizeY()) : LayerDesc.LayerSize;
...
void FPICOXRStereoLayer::PXRLayersCopy_RenderThread(FPICOXRRenderBridge* RenderBridge, FRHICommandListImmediate& RHICmdList)
{
	...
	PXR_LOGV(PxrUnreal, "ID=%d, bTextureNeedUpdate=%d, IsVisible:%d, SwapChain.IsValid=%d, LayerDesc.Texture.IsValid=%d", ID, bTextureNeedUpdate, IsVisible(), SwapChain.IsValid(), LayerDesc.TextureObj.IsValid());

	if (bTextureNeedUpdate && IsVisible())
	{
		// Copy textures
		if (LayerDesc.TextureObj.IsValid()&&LayerDesc.TextureObj->GetResource() && SwapChain.IsValid())
		{
			...
			// Mono
			FRHITexture* SrcTexture = LayerDesc.TextureObj->GetResource()->GetTextureRHI();
			...
			// Stereo
			if (LayerDesc.LeftTextureObj.IsValid() && LeftSwapChain.IsValid())
			{
				FRHITexture* LeftSrcTexture = LayerDesc.LeftTextureObj->GetResource()->GetTextureRHI();
...
bool FPICOXRStereoLayer::InitPXRLayer_RenderThread(const FGameSettings* Settings, FPICOXRRenderBridge* CustomPresent, FDelayDeleteLayerManager* DelayDeletion, FRHICommandListImmediate& RHICmdList, const FPICOXRStereoLayer* InLayer)
{
	...
	else if(bSplashBlackProjectionLayer)
	{
		uint32 SizeX = 1;
		uint32 SizeY = 1;
		if (LayerDesc.TextureObj.IsValid()&&LayerDesc.TextureObj->GetResource())
		{
			FRHITexture* Texture2D = LayerDesc.TextureObj->GetResource()->GetTextureRHI();

			if (Texture2D->GetDesc().Dimension==ETextureDimension::Texture2D)
			{
	...
	else
	{
		...
		if (LayerDesc.TextureObj.IsValid()&&LayerDesc.TextureObj->GetResource())
		{
			FRHITexture* Texture = LayerDesc.TextureObj->GetResource()->GetTextureRHI();
			if (Texture->GetDesc().Dimension==ETextureDimension::Texture2D)
			{
				PxrLayerCreateParam.width = Texture->GetSizeX();
				PxrLayerCreateParam.height = Texture->GetSizeY();
				PxrLayerCreateParam.sampleCount = Texture->GetNumSamples();
				PxrLayerCreateParam.mipmapCount = Texture->GetNumMips();
			}
			else if (Texture->GetDesc().Dimension==ETextureDimension::TextureCube)
			{
				PxrLayerCreateParam.width = PxrLayerCreateParam.height = Texture->GetSize();
				PxrLayerCreateParam.sampleCount = 1;
				PxrLayerCreateParam.mipmapCount = 1;
			}
		...
		PxrLayerCreateParam.layerLayout = LayerDesc.LeftTextureObj.IsValid() ? PXR_LAYER_LAYOUT_STEREO : PXR_LAYER_LAYOUT_MONO;
#if PLATFORM_ANDROID
		 if (CustomPresent->RHIString == TEXT("Vulkan"))
		{
			PxrLayerCreateParam.format = IsMobileColorsRGB() ? VK_FORMAT_R8G8B8A8_SRGB : VK_FORMAT_R8G8B8A8_UNORM;
	...
	if ((LayerDesc.Flags & IStereoLayers::LAYER_FLAG_TEX_CONTINUOUS_UPDATE) && LayerDesc.TextureObj.IsValid() && IsVisible())
	{
		bTextureNeedUpdate = true;
	}
    return true;
