...
int32 UPICOEnterpriseFunctionLibrary::PE_StartActivity(const FString& PackageName, const FString& ClassName, const FString& Action, const FString& Extra, const TArray<FString>& Categories, const TArray<int32>& Flags, int32 Ext)
{
	...
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		...
		std::vector<jint> jint_Flags(FlagSize);
		for (int32 Index = 0; Index < FlagSize; ++Index)
		{
			jint_Flags[Index] = Flags[Index];
		}
		Env->SetIntArrayRegion(jarray_Flags, 0, FlagSize, jint_Flags.data());
...
bool UPICOEnterpriseFunctionLibrary::PE_GetCameraExtrinsics(TArray<double>& leftCameraExtrinsics, TArray<double>& rightCameraExtrinsics)
{
#if PLATFORM_ANDROID
...
bool UPICOEnterpriseFunctionLibrary::PE_GetCameraIntrinsics(int width, int height, double h_fov, double v_fov, TArray<double>& rateArray)
{
#if PLATFORM_ANDROID
