...
bool FOnlineLeaderboardPico::ReadPicoLeaderboards(bool bOnlyFriends, bool bOnlyLoggedInUser, FOnlineLeaderboardReadRef& ReadObject)
{
	...
	const bool UseUEObject = !ReadObject->LeaderboardName.IsEmpty();
	if (UseUEObject)
	{
		PageSize = MAX_REQUEST_SIZE;
		// PageIndex = ReadIndex;
		LeaderboardName = ReadObject->LeaderboardName;
                                               ...
                                               FOnlineLeaderboardWrite& WriteObject)
{
	...
	if (StatData == nullptr)
	{
		SaveLog(ELogVerbosity::Type::Error, FString::Printf(TEXT("WriteLeaderboards Could not find RatedStat: %s"), *WriteObject.RatedStat));
		return false;
	}
