...
bool FOnlineSessionPico::StartSession(FName SessionName)
{
	...
	if (Session == nullptr)
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("PPF_GAME StartSession cannot fine session: %s"), *SessionName.ToString());
		return false;
	}
...
bool FOnlineSessionPico::UpdateRoomDataStore(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings)
{
	...
	{
		if (Setting.Key == SETTING_PICO_POOL)
		{
			continue;
		}
	...
	int index = 0;
	ppfKeyValuePairArray DataStore = ppf_KeyValuePairArray_Create(num);
	for (auto& Setting : UpdatedSessionSettings.Settings)
	{
		if (Setting.Key == SETTING_PICO_POOL)
		{
		...
		if (!ExistingSetting || ExistingSetting->Data != Setting.Value.Data)
		{
			...
			ppf_KeyValuePair_SetKey(item, DataStoreKeys.Last()->Get());
			ppf_KeyValuePair_SetStringValue(item, DataStoreValues.Last()->Get());
			index++;
		}
	}
	for (auto& Setting : Session->SessionSettings.Settings)
	{
		...
		if (!ExistingSetting)
		{
			...
			auto item = ppf_KeyValuePairArray_GetElement(DataStore, index);
			ppf_KeyValuePair_SetKey(item, DataStoreKeys.Last()->Get());
			ppf_KeyValuePair_SetStringValue(item, "");
			index++;
		}
	}
	auto DataStoreSize = num;
	if (DataStoreSize > 0)
	{
		PicoSubsystem.AddAsyncTask(
			ppf_Room_UpdateDataStore(GetRoomIDOfSession(*Session), DataStore, DataStoreSize),
		...
		FPicoMessageOnCompleteDelegate::CreateLambda([this, SearchSettings](ppfMessageHandle Message, bool bIsError)
			{
				if (bIsError)
				{
					UE_LOG_ONLINE(Display, TEXT("PPF_GAME FindMatchmakingSessions OnComplete error"));
					SearchSettings->SearchState = EOnlineAsyncTaskState::Failed;
					TriggerOnFindSessionsCompleteDelegates(false);
					return;
				}
...
TSharedRef<FOnlineSession> FOnlineSessionPico::CreateSessionFromRoom(ppfRoomHandle Room) const
{
	...
	UE_LOG_ONLINE(Display, TEXT("PPF_GAME CreateSessionFromRoom RoomId: %llu, RoomMaxUsers: %d, RoomCurrentUsersSize: %lld, RoomPolicy: %s, RoomType: %s")
		, RoomId, RoomMaxUsers, RoomCurrentUsersSize, *FString(JoinPolicyNames[RoomPolicy]), *FString(RoomTypeNames[RoomType]));
...
void FOnlineSessionPico::LogRoomData(ppfRoomHandle Room) const
{
	...
	auto RoomPolicy = ppf_Room_GetJoinPolicy(Room);
	auto RoomType = ppf_Room_GetType(Room);
	UE_LOG_ONLINE(Display, TEXT("PPF_GAME LogRoomData RoomId: %llu, RoomMaxUsers: %d, RoomCurrentUsersSize: %lld, RoomPolicy: %s, RoomType: %s")
		, RoomId, RoomMaxUsers, RoomCurrentUsersSize, *FString(JoinPolicyNames[RoomPolicy]), *FString(RoomTypeNames[RoomType]));
#endif
}
...
void FOnlineSessionPico::OnNetNotificationConnection(ppfMessageHandle Message, bool bIsError)
{
		...
		|| ConnectionResult == ppfPlatformGameConnectionEvent_KickedByGameServer)
	{
		...
		else
		{
			...
			Uninitialize();
			EAppReturnType::Type ReturnType = FMessageDialog::Open(EAppMsgType::OkCancel, FText::FromString(TEXT("You are Kicked! Click Ok to reinitialize or click Cancel to do nothing. Or you can set the GameConnectionCallback to customize the handling of this message!")));
